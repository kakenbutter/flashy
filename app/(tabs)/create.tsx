      ...
      <KeyboardAwareScrollView bottomOffset={62} style={{ height: "100%" }}>
        ...
        >
          ...
          <Pressable
            ...
            onPress={() => {
              if (title.length === 0 || title.trim().length === 0) {
                ...
                try {
                  let file = new File(Paths.document, title + ".json");
                  for (let i = 1; file.exists; i++) {
                    file = new File(Paths.document, title + i + ".json");
                  }
                  file.create();
                  file.write(
                    JSON.stringify({
          ...
          >
            ...
            <ThemedText type="defaultSemiBold">Create Deck</ThemedText>
          </Pressable>
          ...
          <View style={[styles.groupContainer, { backgroundColor }]}>
            <View style={[styles.textBox, { backgroundColor: secondaryColor }]}>
              ...
              <View
                style={{
                  flexDirection: "row",
                  marginTop: Platform.OS === "ios" ? 8 : undefined,
                  marginHorizontal: Platform.OS === "android" ? 4 : undefined,
                }}
              >
                <ThemedText
                  style={{
...
const styles = StyleSheet.create({
  ...
  textInput: {
    fontSize: 18,
    lineHeight: 21,
    marginBottom: Platform.OS === "ios" ? 8 : undefined,
  },
});
