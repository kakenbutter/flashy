        ...
        {data.length === 0 ? (
          <>
            ...
            <ThemedText type="title">
              Congrats! You have finished this deck.
            </ThemedText>
            <ThemedText>
              You can continue looking through the deck by pressing the "Review"
              button below, or you can return to the home screen.
            </ThemedText>
            <View style={{ flexDirection: "row", gap: 12 }}>
              <Pressable
                ...
                onPress={() => router.replace(`/(flashcard)/review/${path}`)}
              >
                <ThemedText type="defaultSemiBold" style={{ margin: "auto" }}>
                  Review
                </ThemedText>
              </Pressable>
              <Pressable
            ...
            >
              {getItem("hasFlippedCard") ? (
                <>
                  ...
                  >
                    <View
                      style={{ flexDirection: "row", margin: "auto", gap: 8 }}
                    >
                      <ThemedText type="defaultSemiBold">Again</ThemedText>
                      <IconSymbol name="xmark" color={iconColor} />
                    </View>
                  </Pressable>
                  <Pressable
                    style={[styles.button, { backgroundColor }]}
                    onPress={() => {
                      const nextTestData = data.filter(
                        (a) => a !== data[deckIndex],
                      );

                      const nextDeckIndex =
                        deckIndex >= nextTestData.length ? 0 : deckIndex;

                      translateX.value = withSequence(
                        withTiming(2048, { duration: 400 }),
                        withTiming(-2048, { duration: 0 }, () => {
                          runOnJS(setData)(nextTestData);
                          runOnJS(setDeckIndex)(nextDeckIndex);
                          runOnJS(setCardSide)(1);
                        }),
                  ...
                  >
                    <View
                      style={{ flexDirection: "row", margin: "auto", gap: 8 }}
                    >
                      <ThemedText type="defaultSemiBold">Good</ThemedText>
                      <IconSymbol name="checkmark" color={iconColor} />
                    </View>
                  </Pressable>
                </>
              ) : (
